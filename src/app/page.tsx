		...
		<div className="flex items-center justify-center p-4 overflow-clip font-semibold text-lg text-off-white">
			<main className="flex flex-wrap w-full overflow-clip p-4">
				<div className="w-full flex justify-center items-center gap-2 mb-4">
					{/* Hidden real input */}
					<input disabled={loading} id="json-file" type="file" accept=".json" className="hidden" onChange={(e) => setFile(e.target.files?.[0] ?? null)} />
					...
					{file && (
						<div className="w-max flex gap-x-2">
							{/* Submit button */}
							<button className={`bg-off-white rounded-full p-2 text-black transition ${loading ? "opacity-50 cursor-not-allowed" : "hover:cursor-pointer"}`} onClick={handleSubmit} disabled={loading}>
								<FaCheck />
							</button>

							{/* Cancel button */}
							<button className={`bg-off-white rounded-full p-2 text-black transition ${loading ? "opacity-50 cursor-not-allowed" : "hover:cursor-pointer"}`} onClick={() => setFile(null)} disabled={loading}>
								<FaXmark />
							</button>
						</div>
					)}
